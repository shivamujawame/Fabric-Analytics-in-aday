...
4. 在 **主页 (1)** 选项卡上，单击 **新建查询 (2)** 并选择 **空白查询 (3)**。

   ![](../media/Lab04/Chinese-task1-1.png)

5. 单击 **下一步** 创建空白查询。

   ![](../media/Lab04/Chinese-task1-2.png)

6. 在左侧 **查询** 窗格中，您将看到已添加空白查询。

   ![](../media/Lab04/Chinese-task1-3.png)

7. 如果您尚未打开
    **FAIAD.pbix**，请打开它。它位于您的实验室环境的桌面的 **Reports**
    文件夹中。

8. 从功能区中选择**主页 -> 转换数据** 。Power Query
    窗口随即打开。您在之前的实验室中注意到，左侧面板中的查询是按数据源组织的。

9. 从左侧面板的 SharepointData 文件夹下，**选择 People** 查询。
...
12. 在**数据流窗格**中，输入 **Ctrl+V**（目前不支持右键单击"粘贴"）。如果您使用的是 MAC 设备，请使用 Cmd+V 进行粘贴。
...
3. 我们正在使用的数据流是 **Dataflow1** 。在继续下面的步骤之前，我们先将其重命名。 点击数据流 1 旁边的**省略号 (...)** 。选择**属性** 。
...
4. 如果您尚未打开
    **FAIAD.pbix**，请打开它。它位于您的实验室环境的桌面的 **Reports** 文件夹中 。
...
4. **选择"创建新连接 (1)"，**"连接设置"对话框随即打开。输入 **org6c18814a.crm.dynamics.com (2)** 作为**环境域。**
...
Bucket，然后按照类似实验室 3（使用视觉对象查询转换数据并创建视图）的流程操作。我们还可以像之前在本实验室中使用的那样，使用数据流
Gen2 连接到 SharePoint。但是，我们无权访问这些**存储桶/目录** 。

   在我们的应用场景中，IT 团队已建立与 Dataverse
的链接并已应用必要的数据转换， 在 Power BI Desktop
文件中镜像这些数据转换。他们已在管理员工作区中将此数据引入到湖屋中，并向我们提供了对表的访问权限。由于我们的 IT 团队已经完成所有艰苦的工作， 因此我们可以在管理员工作区中创建此湖屋的快捷方式。
